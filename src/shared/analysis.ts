...
${sr ? `
[★ 지원자 특별 요청 — 무엇보다 먼저, 반드시 반영 ★]
${name}님이 직접 남긴 특별 요청: "${sr}"
- 이 요청은 아래 모든 첨삭 관점·원칙보다 우선합니다. 첨삭을 시작하기 전에 요청의 의도를 먼저 파악하고, 작성을 마친 뒤 모든 corrections와 finalAdvice가 이 요청을 충분히 반영했는지 다시 점검하십시오.
- 요청을 직접 충족하는 지적을 반드시 1개 이상 만들어 corrections 배열의 맨 앞에 배치하고, 해당 항목의 isSpecialRequestRelated를 true로 설정하십시오(요청과 무관한 항목은 false).
- 요청이 특정 내용의 추가·강조·삭제·톤(말투/분위기) 변경을 의미하면, 서류에 근거가 있는 범위 내에서 그 의도를 corrected 문장에 실제로 구현하십시오(환각 금지 원칙은 그대로 유지).
- 요청의 의도가 모호하면 가장 합리적인 방향으로 해석하되, 어떻게 해석해 반영했는지 reason에서 "${name}님" 호칭으로 한 문장 밝히십시오.
` : ""}
[최우선 원칙 — 반드시 준수]
...
[corrected 작성 원칙 — 사람 문체로, AI 탐지 회피]
corrected는 ${name}님이 그대로 제출할 문장이므로, AI 문체 판별기(GPTZero·카피킬러 등)에서 'AI가 작성한 글'로 분류되지 않도록 사람이 직접 쓴 듯 자연스럽게 작성하십시오.
- 문장 길이와 구조에 변화를 주십시오. 짧은 문장과 긴 문장을 섞고, 모든 문장을 똑같은 패턴(예: "~을 통해 ~할 수 있었습니다")으로 끝맺지 마십시오.
- AI가 즐겨 쓰는 상투어·접속어 남발을 피하십시오(예: "또한", "뿐만 아니라", "나아가", "이를 통해", "적극적으로", "다양한", "체계적으로"의 기계적 반복).
- 지나치게 매끄럽고 대칭적인 나열·병렬 구조를 피하고, 실제 지원자가 쓰듯 구체적이고 개별적인 사실·맥락·숫자를 담으십시오.
- 추상적 미사여구 대신 서류에 실재하는 구체적 행동·상황·결과를 1인칭으로 서술하십시오(환각 금지).
- 단, 비속어·오탈자·문법 오류를 일부러 넣지는 마십시오. 어디까지나 '자연스럽게 정돈된 사람의 글'이어야 합니다.
...
- corrections: 수정 항목 배열
  ...
  - reason: 왜 고쳐야 하는지 — 평가위원 관점, 채용 담당자가 기피하는 표현의 원인, 개선의 비즈니스 효과를 "${name}님" 호칭으로 설득력 있게 서술하십시오. (필요한 깊이만큼 충실히 쓰되, 형식적인 분량 채우기는 금지)
    내용이 길어지면 논점이 바뀌는 지점마다 빈 줄을 넣어 문단을 나눠 가독성을 높이고, 핵심 어구나 작은 소제목을 붙일 때는 그 글자만 별표 두 개로 감싸(**핵심**) 굵게 표시하십시오.
...
- finalAdvice: 다음 소제목을 각각 대괄호로 감싸 한 줄에 단독 표기하고, 그 아래에 내용을 서술하십시오.
  ...
  - 각 소제목 아래의 내용은 한 덩어리로 몰아쓰지 말고, 논점이 바뀔 때마다 빈 줄을 넣어 문단을 나눠 가독성을 높이십시오.
  - 문단 안에서 핵심 어구를 강조하거나 작은 소제목(예: 예상 면접 질문, 개발이 필요한 역량)을 붙일 때는 그 글자만 별표 두 개로 감싸(**예상 면접 질문**) 굵게 표시하십시오.
  - [향후 보완 전략]에는 예상 면접 질문과 개발이 필요한 역량을 포함하십시오.
  - 호칭은 오직 "${name}님" 형태만 사용하십시오.
  - 강조는 위의 별표 두 개 방식만 사용하고, 홑따옴표(')나 점수·등급 등 어떤 형태의 평가 점수도 포함하지 마십시오.
  - '\\n\\n' 같은 이스케이프 문자를 그대로 출력하지 말고 실제 줄바꿈을 사용하십시오.
`;
}

export function buildVerifyPrompt(opts: { name: string; specialRequest?: string }): string {
  const name = opts.name.trim();
  const sr = (opts.specialRequest || "").trim();
  return `
...
6. 사람 문체(AI 탐지 회피): corrected가 AI 문체로 보이면 사람이 쓴 듯 다듬으십시오. 문장 길이·구조에 변화를 주고, 똑같은 어미 패턴 반복과 "또한/뿐만 아니라/이를 통해/적극적으로/다양한" 같은 상투어 남발을 줄이며, 지나친 대칭·나열을 풀어 자연스럽게 만드십시오(환각 추가 금지).${sr ? `
7. 특별 요청 반영 점검: ${name}님의 특별 요청("${sr}")이 충분히 반영됐는지 확인하십시오. 반영이 약하면 요청을 직접 충족하는 corrections를 보강해 배열 앞쪽에 두고 isSpecialRequestRelated를 true로 설정하며, 요청 의도가 corrected에 실제로 구현됐는지 점검하십시오(서류 근거 범위 내, 환각 금지).` : ""}

기존 규칙(진실성, 점수 미표기, 홑따옴표 금지, 강조는 별표 두 개(**)로 표기, 문단 나누기, 실제 줄바꿈)은 그대로 유지하며, 내용의 품질만 향상시킨 최종본을 반환하십시오.
`;
}
